...
# class RootWidget(BoxLayout):
    ...
    # label_txt = StringProperty("Polling for tx...")
    # btn_ok_txt= StringProperty("wait_ok")
    # btn_cancel_txt= StringProperty("wait_cancel")
    # btn_ok_disabled= BooleanProperty(True)
    # btn_cancel_disabled= BooleanProperty(True)
    ...
    # def update(self, dt):
        ...
        # for keyhash in self.listener.keyhashes:
            ...
            # if message:
                ...
                # pre_tx_dic= cryptos.deserialize(pre_tx_hex)
                # print("pre_tx_dic: ", pre_tx_dic)
                # label_txt = str(pre_tx_dic)
                
                # # compute tx_hash
                # #pre_hash= bytes.fromhex(message)
                # pre_hash= sha256(pre_tx).digest()
                # pre_hash= sha256(pre_hash).digest()
                # #pre_hash= pre_hash+ (b'\0'*32) # 32bytes zero-padding
                # pre_hash= pre_hash+ (b'\1'*32) # 32bytes zero-padding
                # pre_hash_hex= pre_hash.hex()
                # print("tx_hash: ", pre_hash_hex)
                # #compute  response to challenge and send back...
                # mac = hmac.new(secret_2FA, pre_hash, sha1)
            ...
            # else: 
                # self.label_txt = "Here is the tx: xxxxx to yyyyyyy"
                # self.btn_ok_txt= "Approve tx"
                # self.btn_cancel_txt= "Reject tx"
                # self.btn_ok_disabled= False
                # self.btn_cancel_disabled= False
                # print("Do nothing!")
